...
StatsFile="/var/run/ethos/stats.file"
...
MinHashRate=25 # SET MINIMUM HASH RATE
MinMem=2100 # SET MINIMUM WATTS
RebootMaxRestarts=5 # REBOOT IF THERE ARE MORE THEN X RESTARTS WITHIN 1H
Miner=$(cat "$StatsFile" | grep miner: | cut -d ":" -f 2)
MinerSeconds=$(cat "$StatsFile" | grep miner_secs: | cut -d ":" -f 2)
...
YellowEcho "Minimum Memory: $MinMem"
...
function Json2Array() {
	Index=0
	x=' ' read -r -a Values <<< "`cat "$StatsFile" | grep "${1}:" | cut -d ":" -f 2`"
	if [[ $Values != "null" ]]; then
		for Value in "${Values[@]}"
		do
			eval "$1[$Index]"="$Value"
...
function getMinerHashes() {
	Index=0
	x=' ' read -r -a Values <<< "`cat "$StatsFile" | grep "miner_hashes:" | grep -v "dual" | cut -d ":" -f 2`"
	if [[ $Values != "null" ]]; then
		for Value in "${Values[@]}"
		do
			eval "$1[$Index]"="$Value"
...
function getMinerMems() {
	Index=0
	x=' ' read -r -a Values <<< "`cat "$StatsFile" | grep mem: | grep -v default | cut -d ":" -f 2`"
	if [[ $Values != "null" ]]; then
		for Value in "${Values[@]}"
		do
			eval "$1[$Index]"="$Value"
...
if [[ $MinerSeconds -gt 300 ]]; then
	...
	do
		if [[ "${miner_hashes[$Index]/.*}" -lt $MinHashRate ]]; then
			...
			RestartMiner
		elif [[ "${mems[$Index]/.*}" -lt $MinMem ]]; then
			RedEcho "$(date "+%d.%m.%Y %T") RESTART: GPU[$Index] MEM:${mems[$Index]}.[Miner was running for: $MinerTime]" | tee -a "$LogFile"
			RestartMiner
		else
			GreenEcho "STATUS OK: GPU[$Index] HASH:${miner_hashes[$Index]} MEM:${mems[$Index]}"
		fi
	    let Index++
	done
else
